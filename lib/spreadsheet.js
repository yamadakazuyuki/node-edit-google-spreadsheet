...
//public api
exports.create = exports.load = util.promisify(function(opts, callback) {
  if (!callback) {
...
Spreadsheet.prototype.request = util.promisify(function(opts, callback) {
  ...
  request(opts, function(err, response, body) {
    ...
    if (
      ...
      typeof _this.opts.accessToken !== "object"
    ) {
      _this.log(
        "Authentication token expired. Logging into Google again...".grey
      );
...
Spreadsheet.prototype.getSheetId = util.promisify(function(type, callback) {
  var _this = this;
  var id = type + "sheetId";
  ...
  this.request(
    ...
    function(err, result) {
      ...
      _this.raw[type + "sheets"] = entries.map(function(e1) {
        ...
        for (var prop in e1) {
          ...
          if (typeof val === "object") {
            ...
            if (keys.length === 1 && keys[0] === "$t") val = val.$t;
          }
          //remove silly gs$
      ...
      var m = null;
      if (!_this[id]) return callback(type + "sheet '" + name + "' not found");
      ...
      _this.log(
        ("Tip: Use option '" +
          ...
          'sheetId: "' +
          _this[id] +
          "\"' for improved performance").yellow
      );
      callback(null);
    }
  );
...
Spreadsheet.prototype.add = function(cells) {
  //init data
  if (_.isArray(cells)) this.arr(cells, 0, 0);
  else this.obj(cells, 0, 0);
};
...
Spreadsheet.prototype.obj = function(obj, ro, co) {
  ...
  for (row in obj) {
    ...
    for (col in cols) {
      col = util.num(col);
      var data = cols[col];
      if (_.isArray(data)) this.arr(data, row - 1 + ro, col - 1 + co);
      else this.addVal(data, row + ro, col + co);
    }
  }
};
...
Spreadsheet.prototype.addVal = function(val, row, col) {
  ...
  if (t === "string" || t === "number" || t === "boolean") obj.val = val;
  else obj = _.extend(obj, val);

  if (obj.name)
    if (this.names[obj.name]) throw "Name already exists: " + obj.name;
    else this.names[obj.name] = obj;
...
Spreadsheet.prototype.compile = function() {
  ...
  for (row in this.entries)
    for (col in this.entries[row]) {
      ...
      if (typeof obj.val === "string") obj.val = this.getNames(obj);

      if (obj.val === undefined) continue;
      else obj.val = _.escape(obj.val.toString());
...
//send (bulk) changes
Spreadsheet.prototype.send = util.promisify(function(options, callback) {
  ...
  if (!this.bodyTemplate || !this.entryTemplate)
    return callback(
      "No templates have been created. Use setTemplates() first."
    );
  ...
  async.series(
    [
      function autoSize(next) {
        if (!options.autoSize) return next();
        ...
        _this.metadata(function(err, metadata) {
          ...
          //no resize needed
          if (
            metadata.rowCount >= _this.maxRow &&
      ...
      function send(next) {
        _this.request(
          ...
          function(err, result) {
            ...
            if (!result.feed)
              return next(
                "Google Spreadsheets API has changed please post an issue!"
              );
            ...
            var entries = _.isArray(result.feed.entry)
              ? result.feed.entry
              : [result.feed.entry];
              ...
              var msg =
                ...
                errors
                  .map(function(e, i) {
                    ...
                    if (
                      !msg ||
                      /Please wait a bit and try reloading your spreadsheet/.test(
                        msg
                      )
                    )
                      msg =
                        "Your update may not fit in the worksheet. See the 'autoSize' option.";
                    return msg;
                  })
                  .join(" ");
...
Spreadsheet.prototype.receive = util.promisify(function(options, callback) {
  if (typeof options === "function") {
    callback = options;
    options = {};
  }
  ...
  this.request(
    ...
    function(err, result) {
      ...
      var info = {
        ...
        worksheetUpdated:
          new Date(result.feed.updated.$t || result.feed.updated) || null,
      ...
      _.each(entries, function(entry) {
        ...
        rows[r][c] = util.gcell2cell(
          cell,
          options.getValues,
          _this.opts.useCellTextValues
        );

        if (options.isTrimmed) {
          rows[r][c] = rows[r][c] && rows[r][c].replace(/^[\s|　]+|[\s|　]+$/g, '');
        }

        info.lastRow = util.num(r);
      });

      if (entries.length) info.nextRow = info.lastRow + 1;
...
Spreadsheet.prototype.metadata = util.promisify(function(data, callback) {
